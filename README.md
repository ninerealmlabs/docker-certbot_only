...
Certbot_Only is a docker image based off of [linuxserver's SWAG](https://linuxserver.io) with the goal to simplify the image to _only generate DNS certificates and maintain them_ while leaving them accessible for other resources to utilize.
Because Certbot_Only _only runs certbot and a monitoring cron task_, DNS validation is required.
